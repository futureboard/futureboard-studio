    ...
    pub(super) fn dispatch_transport_command(
        ...
        match command {
            TransportCommand::PlayPause => {
                ...
                // Space controls playback only. Record is deliberately an
                // explicit action: an armed track must not turn an ordinary
                // Play press into a new take after the previous recording has
                // finished. This keeps the lifecycle one-shot until the user
                // presses R again.
                let playing = self
                    .audio_bridge
                    .stats
